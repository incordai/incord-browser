...
```bash
incord-browser fetch https://example.com --stealth
incord-browser serve --stealth
incord-browser scrape url1 url2 --stealth
incord-browser mcp --stealth
```
...
```bash
incord-browser fetch https://example.com --proxy http://proxy.example.com:8080
incord-browser serve --proxy http://proxy.example.com:8080
```

With auth:

```bash
incord-browser fetch https://example.com --proxy http://user:pass@proxy.example.com:8080
```

SOCKS5:

```bash
incord-browser fetch https://example.com --proxy socks5://proxy.example.com:1080
```

## Custom User-Agent

```bash
incord-browser fetch https://example.com --user-agent "Mozilla/5.0 (...) ..."
incord-browser serve --user-agent "Mozilla/5.0 (...) ..."
```
...
```bash
OBSCURA_PROFILE=2 incord-browser serve          # pin a specific profile by index
OBSCURA_ROTATE_PROFILE=1 incord-browser serve   # random profile per browser context
```
